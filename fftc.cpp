...
func.func private @fft_memref(%arg0: memref<?x?xf64>, %arg1: memref<?x?xf64>) -> i64
...
//      %stride1_d1: i64,
//
void fft_memref(double *allocated_ptr0, double *aligned_ptr0,
                     intptr_t offset0, intptr_t size0_d0, intptr_t size0_d1,
                     intptr_t size0_d2, intptr_t stride0_d0,
                     intptr_t stride0_d1, intptr_t stride0_d2,
                     // Second Memref (%arg1)
                     double *allocated_ptr1, double *aligned_ptr1,
                     intptr_t offset1, intptr_t size1_d0, intptr_t size1_d1,
                     intptr_t size1_d2, intptr_t stride1_d0,
                     intptr_t stride1_d1, intptr_t stride1_d2);

// The llvm.emit_c_interface will also trigger emission of another wrapper:
// llvm.func @_mlir_ciface_fft_memref(
...
void _mlir_ciface_fft_memref(memref_3d_descriptor *arg0,
                                  memref_3d_descriptor *arg1);
...
void dump() {
  for (int i = 0; i < N; i++) {
    ...
    for (int j = 0; j < M; j++) {
      for (int k = 0; k < L; k++)
        printf("%d,\t", (int)arg0[i][j][k]);
      printf("] [");
    }
    for (int j = 0; j < M; j++) {
      for (int k = 0; k < L; k++)
        printf("%d,\t", (int)arg1[i][j][k]);
      printf("]\n");
    }
  }
}
...
int main() {
 ...
 // printf("Before:\n");
  ...
  // Call into MLIR.
  fft_memref((double *)arg0, (double *)arg0, 0, N, M, L, L, M, 0,
                                //
                                (double *)arg1, (double *)arg1, 0, N, M, L, L, M, 0);
 ...
 // dump();
  ...
  // Call into MLIR.
  memref_3d_descriptor arg0_descriptor = {
      (double *)arg0, (double *)arg0, 0, N, M, L, L, M, 0};
  memref_3d_descriptor arg1_descriptor = {
      (double *)arg1, (double *)arg1, 0, N, M, L, L, M, 0};
  _mlir_ciface_fft_memref(&arg0_descriptor, &arg1_descriptor);
